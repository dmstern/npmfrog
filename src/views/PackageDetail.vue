            ...
            <v-tabs v-model="activeTab">
              ...
              <v-tab-item class="readme">
                <v-card-text>
                  <div
                    v-highlightjs
                    v-if="data.packageDetail.readme"
                    :key="data.packageDetail.readme"
                    v-html="data.packageDetail.readme"
                  ></div>
                  <div v-else>
                    <v-alert :value="true" type="warning">No README.md file found.</v-alert>
                  </div>
                </v-card-text>
              </v-tab-item>
              <v-tab-item>
                    ...
                    <table class="package-detail__scripts">
                      ...
                      <tbody>
                        <tr v-for="(script, key) in data.currentPackage.scripts" :key="key">
                          <td>
                            <CodeBlock
                              :code="['start', 'test', 'version'].some(task => task === key) ? `npm ${key}` : `npm run ${key}`"
                              language="bash"
                            ></CodeBlock>
                          </td>
                          <td>
                  ...
                  <div v-if="data.showFiles">
                    ...
                    <div @click="selectCode">
                      ...
                      >
                        <template slot="prepend" slot-scope="{ item, open, leaf }">
                          <v-icon
                            v-if="item.children"
                          >{{ open ? $vuetify.icons.folderOpen : $vuetify.icons.folder }}</v-icon>
                          <v-icon v-else>{{ getFileIcon(item.name) }}</v-icon>
                        </template>
                        <template
                          slot="append"
                          slot-scope="{ item }"
                        ...
                        >
                          <v-list-tile-action>{{ fileSize(item.size) }}</v-list-tile-action>
                        </template>
                      </v-treeview>
                    </div>
                    <v-alert
                    ...
                    >
                      <h3>
                        ...
                        >This file is too big to show it's content (> {{maxSizeToShowContent / 1000}} kB).</span>
                        <span
                          v-if="!isHighlightableType(data.activeTreeItem)"
                        >This filetype ({{`${data.activeTreeItem.type}`}}) is not supported.</span>
                      </h3>But you can download it here:
                      <ExternalLink
                    ...
                    <LoadingSpinner class="transition" :class="isLoadingCode? 'visible' : 'hidden'"></LoadingSpinner>
                    <CodeBlock
                ...
                <v-card-text v-else>
                  <v-alert :value="true" type="info">Nothing to show...</v-alert>
                </v-card-text>
              </v-tab-item>
              <v-tab-item>
                <v-card-text v-if="data.currentPackage.dependenciesCount">
                  <v-alert type="info" :value="true" outline dismissible>
                  ...
                  <router-link
                    ...
                    :to="`/package/${key}`"
                  >
                    <v-chip small outline color="primary">{{key}}</v-chip>
                  </router-link>
                ...
                <v-card-text v-else>
                  <v-alert
                    ...
                    type="info"
                    :outline="true"
                  >This package has no dependencies.</v-alert>
                </v-card-text>
              </v-tab-item>
              <v-tab-item>
                <v-card-text>
                  <v-alert type="info" :value="true" :outline="true" dismissible>
        ...
        <v-flex xs12 md5 xl4 order-xs1 order-md2 class="meta-panel">
          <v-layout row wrap>
            <PackageDetailItem
              title="install"
              :bigContent="false"
              v-if="data.config && data.config.artifactory"
              :icon="$vuetify.icons.install"
              :full="true"
            >
              <CodeBlock
                title="Put this in a .npmrc file in your project folder:"
                :code="getInstallCode().npmrc"
                language="properties"
              ></CodeBlock>
              <div class="config-info">
                <ExternalLink
                  href="https://docs.npmjs.com/misc/config#npmrc-files"
                  :icon="$vuetify.icons.question"
                  :text="false"
                  title="More info about npm-config"
                ></ExternalLink>
              </div>
            ...
            <PackageDetailItem
              title="License"
              v-if="data.currentPackage.license"
              :icon="$vuetify.icons.legal"
            >
              <span>{{data.currentPackage.license}}</span>
            </PackageDetailItem>
            <PackageDetailItem
              title="Website"
              v-if="data.currentPackage.homepage"
              :icon="$vuetify.icons.homepage"
            >
            ...
            <PackageDetailItem
              title="Repository"
              v-if="data.currentPackage.repository"
              :icon="$vuetify.icons.code"
            >
              <a :href="`${data.currentPackage.repositoryUrl }`" target="_blank">
                <v-icon
                  v-if="data.currentPackage.repositoryName in $vuetify.icons"
                >{{$vuetify.icons[data.currentPackage.repositoryName]}}</v-icon>
            ...
            >
              <a
                :href="`${data.currentPackage.bugTrackerUrl }`"
                target="_blank"
                title="go to / report issues"
              >
                <v-icon>{{$vuetify.icons.bug}}</v-icon>
              </a>
              <a
                :href="`mailto:${data.currentPackage.bugs.email }`"
                v-if="data.currentPackage.bugs.email"
                target="_blank"
                title="write email"
              >
            ...
            <PackageDetailItem
              title="Crafted by"
              v-if="data.currentPackage.author"
              :icon="$vuetify.icons.author"
              :bigContent="false"
            >
              <v-menu
                open-on-hover
                ...
                offset-y
              >
                <CrafterAvatar :crafter="crafter" slot="activator"></CrafterAvatar>
                <v-card>
                  <v-list>
                    <v-list-tile avatar>
                      <v-list-tile-avatar>
                      ...
                      <v-list-tile-content>
                        ...
                        <v-list-tile-sub-title v-if="crafter.url">
                          <ExternalLink :href="crafter.url" :icon="$vuetify.icons.website"></ExternalLink>
                  ...
                  <v-list>
                    <v-list-tile
                      v-if="inSearchableList(crafter)"
                      @click="triggerSearchFilter(crafter)"
                    >
                      <v-list-tile-action>
...
export default class PackageDetail extends Vue {
  ...
  private getPackageDetails(
    ...
    return DataStore.Instance.getPackageDetail({
      ...
      this.$nextTick(() => {
        this.data.showFiles = response.packageDetail.fileList !== undefined;
      });
      return {
